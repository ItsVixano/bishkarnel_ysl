 ...
 *
 */
#include <linux/types.h> include <linux/slab.h> include <linux/kernel_stat.h> include "cpufreq_governor.h"
/* Lionfish governor tunable defaults */
#define DEF_FREQUENCY_JUMP_THRESHOLD (95) define DEF_FREQUENCY_UP_THRESHOLD (80) define DEF_FREQUENCY_DOWN_THRESHOLD (40) define DEF_FREQUENCY_JUMP_LEVEL (800000) define DEF_SAMPLING_UP_FACTOR (2) define DEF_SAMPLING_DOWN_FACTOR	(3)
/* Lionfish governor fixed settings */
#define RAMP_UP_PERCENTAGE (130) define RAMP_DOWN_PERCENTAGE (65) define FREQUENCY_STEP_PERCENTAGE (5) define JUMP_HISPEED_FREQ_PERCENTAGE (83) define MAX_SAMPLING_FACTOR (10) define LIONFISH_VERSION_MAJOR (1) define LIONFISH_VERSION_MINOR (1)
/************************** type definitions ****************************/ struct lf_cpu_dbs_info_s {
...
struct lf_gdbs_data; /* Common Governor data across policies */ struct lf_dbs_data {
	/*
...
/* Governor per policy data */ struct lf_gdbs_data {
...
/************************ function declarations *************************/ static void lf_gov_queue_work(struct lf_gdbs_data *dbs_data,
		struct cpufreq_policy *policy, unsigned int delay, bool all_cpus); /************************** sysfs macro magic ***************************/
#define lf_show_one(_gov, file_name) \
static ssize_t show_##file_name##_gov_sys \ (struct kobject *kobj, struct attribute *attr, char *buf) \ { \
	struct _gov##_dbs_tuners *tuners = _gov##_dbs_cdata.gdbs_data->tuners; \
	return sprintf(buf, "%u\n", tuners->file_name); \
}									\
									\ static ssize_t show_##file_name##_gov_pol \ (struct cpufreq_policy *policy, char *buf) \ { \
	struct lf_gdbs_data *dbs_data = policy->governor_data; \
	struct _gov##_dbs_tuners *tuners = dbs_data->tuners; \
	return sprintf(buf, "%u\n", tuners->file_name); \
}
#define lf_store_one(_gov, file_name) \
static ssize_t store_##file_name##_gov_sys \ (struct kobject *kobj, struct attribute *attr, const char *buf, size_t count) \ { \
	struct lf_gdbs_data *dbs_data = _gov##_dbs_cdata.gdbs_data;	\
	return store_##file_name(dbs_data, buf, count); \
}									\
									\ static ssize_t store_##file_name##_gov_pol \ (struct cpufreq_policy *policy, const char *buf, size_t count) \ { \
	struct lf_gdbs_data *dbs_data = policy->governor_data; \
	return store_##file_name(dbs_data, buf, count); \
}
#define lf_show_store_one(_gov, file_name) \
lf_show_one(_gov, file_name); \ lf_store_one(_gov, file_name)
#define lf_declare_show_sampling_rate_min(_gov) \
static ssize_t show_sampling_rate_min_gov_sys \ (struct kobject *kobj, struct attribute *attr, char *buf) \ { \
	struct lf_gdbs_data *dbs_data = _gov##_dbs_cdata.gdbs_data;	\
	return sprintf(buf, "%u\n", dbs_data->min_sampling_rate);	\
}									\
									\ static ssize_t show_sampling_rate_min_gov_pol \ (struct cpufreq_policy *policy, char *buf) \ { \
	struct lf_gdbs_data *dbs_data = policy->governor_data; \
	return sprintf(buf, "%u\n", dbs_data->min_sampling_rate);	\
}
/************************** global variables ****************************/ static DEFINE_PER_CPU(struct lf_cpu_dbs_info_s, lf_cpu_dbs_info); static struct lf_dbs_data lf_dbs_cdata; /* there are more globals declared after the sysfs code */ /*********************** lionfish governor logic ************************/ static inline unsigned int get_freq_target(struct lf_dbs_tuners *lf_tuners,
...
/* Computes the maximum absolute load for the policy */ static unsigned int lf_get_load(cpumask_var_t cpus, unsigned int sampling_rate,
...
/*************************** sysfs interface ****************************/ static ssize_t store_sampling_up_factor(struct lf_gdbs_data *dbs_data,
...
/* macros to autogenerate sysfs interface */ lf_show_store_one(lf, sampling_rate); lf_show_store_one(lf, sampling_up_factor); lf_show_store_one(lf, sampling_down_factor); lf_show_store_one(lf, jump_threshold); lf_show_store_one(lf, up_threshold); lf_show_store_one(lf, down_threshold); lf_show_store_one(lf, jump_level); lf_show_store_one(lf, ignore_nice_load); lf_declare_show_sampling_rate_min(lf); gov_sys_pol_attr_rw(sampling_rate); gov_sys_pol_attr_rw(sampling_up_factor); gov_sys_pol_attr_rw(sampling_down_factor); gov_sys_pol_attr_rw(jump_threshold); gov_sys_pol_attr_rw(up_threshold); gov_sys_pol_attr_rw(down_threshold); gov_sys_pol_attr_rw(jump_level); gov_sys_pol_attr_rw(ignore_nice_load); gov_sys_pol_attr_ro(sampling_rate_min); /********** globals requiring sysfs functions to be defined *************/ static struct attribute *dbs_attributes_gov_sys[] = {
...
/*************************** boilerplate code ***************************/ static struct attribute_group *get_sysfs_attr(struct lf_gdbs_data *dbs_data) {
...
MODULE_AUTHOR("Sultan Qasim Khan <sultanqasim@gmail.com>"); MODULE_DESCRIPTION("'cpufreq_lionfish' - A dynamic cpufreq governor for mobile "
		"devices designed to keep CPU frequencies to a minimum while "
		"still briefly boosting frequencies as required to avoid lag." ); MODULE_LICENSE("GPL");
